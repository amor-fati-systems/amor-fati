...
| --- | --- | --- | --- | --- | --- |
| National accounts | GUS national accounts; Eurostat `namq_10_gdp`, `namq_10_a10`, supply-use and input-output tables. | Initialization scale, GDP denominator, sector shares, validation targets. | `pop.realGdp`, `gdpRatio`, `sectorDefs.share`, `MonthlyGdpProxy`, `AnnualizedGdpProxy`, sector `*_Output` validation columns. | Quarterly GDP to monthly runtime denominator; annual sector shares to sector initialization; time-series TSV macro PLN outputs are already emitted in Poland scale; real/nominal split documented per target. | Empirical source extraction and real/nominal conventions still need manifest-backed implementation. Priority: high. |
...
| Banking and credit | NBP MFI balance sheet, MIR interest rates, KNF monthly banking data. | Bank opening stocks, credit calibration, prudential validation, spread calibration. | `banking.initDeposits`, `initLoans`, `openingBankCapitalAggregateTarget`, `initGovBonds`, `baseSpread`, `MinBankCAR`, `MinBankLCR`, `MinBankNSFR`, `NPL`. | Monthly stocks to opening/month-end values; regulatory ratios direct; MIR rates converted to annual/monthly conventions used by the model. | Maturity distributions, recovery rates, and bank-level heterogeneity need better empirical backing. Priority: high. |
| Labor market and wages | GUS LFS/BAEL, registered unemployment, employment, wage and salary series; Eurostat LFS comparators. | Wage calibration, unemployment validation, labor mobility, sector composition. | `household.baseWage`, `labor.frictionMatrix`, `labor.unionDensity`, unemployment columns, regional unemployment columns, `MarketWage`. | Monthly wage series direct; quarterly LFS/unemployment values are held constant across the three months of the quarter for monthly Stage B decisions and runtime unemployment/labor-rate fields; `household.baseWage` uses the configured monthly wage bridge when month-specific wage inputs are available; sector wage indices mapped to six model sectors. Comparisons use quarterly aggregates or quarter-average monthly model values against quarterly LFS rather than month-by-month interpolation. | Cross-sector transition/friction matrix and terminal wage distribution output need stronger support. Priority: high. |
| Firms and production | GUS enterprise demography, REGON/BDL, F-01/non-financial enterprise results, supply-use tables. | Firm-size distribution, entry/exit, revenue/costs, capital ratios, sector productivity. | `pop.firmSizeDist`, `pop.firmSize*Share`, `firm.replacementEntryRate`, `firm.netEntryRate`, `firm.baseRevenue`, `capital.klRatios`, `capital.depRates`, `FirmDeaths`, `FirmBirths`, terminal `_firms.tsv`, sector `*_Output` columns. | Annual/quarterly firm results mapped to six sectors; per-worker and per-firm normalizers recorded explicitly. | Empirical firm demography and sector-output extraction still need manifest-backed implementation. Priority: high. |
...
| Housing and mortgages | NBP housing prices, KNF mortgage data, GUS living-condition and housing tables. | Housing price validation, mortgage stock/defaults, household balance-sheet stress. | `HousingPriceIndex`, regional HPI columns, `MortgageStock`, `MortgageOrigination`, `MortgageRepayment`, `MortgageDefault`, `MortgageNetStockFlow`, `MortgageToGdp`. | Monthly/quarterly HPI rebased to model start; mortgage stocks are month-end; defaults use native cadence where available. Mortgage/GDP path checks use `AnnualizedGdpProxy` to separate numerator runoff from denominator growth. | Regional mapping and borrower-distribution calibration need more detail. Priority: medium. |
...
timeseries and terminal summary TSVs to produce a baseline validation snapshot.

## Priority Gaps

| Priority | Gap | Why it matters | Expected owner surface |
| --- | --- | --- | --- |
| High | Manifest-backed GDP and sector-output empirical comparators. | The model now emits direct GDP proxy and sector-output columns, but publication-grade fit still needs source vintage, real/nominal convention, and transformation metadata. | Empirical validation manifest and report. |
...
| High | Firm demography and production empirical bridge. | ABM meso validation now has terminal firm-size and sector-output surfaces, but still needs GUS/REGON/BDL source extraction and transformation metadata. | Empirical validation manifest and report. |
| High | Credit maturity, recovery, and default/loss records. | Banking stress paths depend on loss timing, recovery, and maturity structure. | Banking config, KNF/NBP bridge rows. |
...
| Medium | Manifest-backed source extraction for scenario-specific shocks. | Policy and shock experiments now expose executable provenance, but publication use still needs source URLs, dataset codes, and extraction manifests where an external path is claimed. | Scenario registry and future data-extract workflow. |
