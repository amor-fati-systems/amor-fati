...
2. Populate it from same-month execution in
   [`MonthFlowPlanBuilder`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/engine/flows/MonthFlowPlanBuilder.scala).
3. Add the mechanism to
   [`FlowMechanism`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/engine/flows/FlowMechanism.scala)
   if it is a new named economic flow.
4. Emit batches in the appropriate [`engine/flows/*Flows.scala`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/engine/flows) module, or create a new one
   under `engine/flows/` if the ownership boundary is new.
5. Wire the emitter through
   [`MonthFlowEmitter`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/engine/flows/MonthFlowEmitter.scala).
...
7. If the flow affects supported persisted stocks, update
   ...
   as needed.
8. Update
   [`SfcSemanticProjection`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/engine/flows/SfcSemanticProjection.scala)
...
Do not compute new behavior inside a [`engine/flows/*Flows.scala`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/engine/flows) emitter. The emitter should
...
3. Insert the stage in
   [`MonthCalculusRunner`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/engine/flows/MonthCalculusRunner.scala)
...
1. Start with the [agents package README](../../modules/model/src/main/scala/com/boombustgroup/amorfati/agents/README.md)
...
1. Add or update scenario configuration in [`config/ScenarioRegistry.scala`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/config/ScenarioRegistry.scala).
2. Add tests under `modules/model/src/test/scala/com/boombustgroup/amorfati/config/`.
...
2. Add an sbt `inputKey` in `build.sbt` if it should be runnable as a named
   repository task.
3. Add focused exporter tests under `modules/cli/src/test/scala/com/boombustgroup/amorfati/diagnostics/`.
...
3. Add or update schema tests under `modules/montecarlo/src/test/scala/com/boombustgroup/amorfati/montecarlo/timeseries/`.
...
1. Add a provider under `modules/model/src/main/scala/com/boombustgroup/amorfati/fp/` using
...
3. Define cross-type operations centrally in [`types.scala`](../../modules/model/src/main/scala/com/boombustgroup/amorfati/types.scala) when the operation
   is meaningful across domains.
4. Add focused coverage in
   [`OpaqueTypesSpec`](../../modules/model/src/test/scala/com/boombustgroup/amorfati/OpaqueTypesSpec.scala)
