...
| `firm.productivityGrowth` | `0.030` | annual rate | Poland 2026-04-30 forward baseline calibration | Baseline productivity/catch-up trend; reduced from the prior catch-up setting after 48m diagnostics showed excessive real GDP acceleration once the production baseline was re-anchored to 2026-04-30 | `.monthly` in use | `FirmConfig` | `TUNED_NEEDS_VALIDATION` |
...
| `firm.netEntryRate`, `netEntryMaxMonthly` | `0.12`, `175` | monthly share / firms | #461 labor/GDP calibration | Demand-backed net births constrained by staffing feasibility; high unemployment is labor availability, not a standalone entry impulse | Direct | `FirmConfig`, `FirmEntry` | `TUNED_NEEDS_VALIDATION` |
