...
class MonetaryPlumbingSpec extends AnyFlatSpec with Matchers:
  ...
  "Banking.reserveInterest" should "compute monthly interest on reserves" in {
    val (bank, bankStocks) = mkBankRow(0, reservesAtNbp = PLN(100000000)) // 100M in reserves
    val interest           = Banking.reserveInterest(bank, bankStocks, Rate.decimal(575, 4))
    // Expected: 100M × 0.0575 × 0.5 / 12 ≈ 239,583
    decimal(interest) shouldBe (BigDecimal("1e8") * BigDecimal("0.0575") * BigDecimal("0.5") / BigDecimal("12.0") +- BigDecimal("1000.0"))
  }

  it should "return 0 for failed banks" in {
    val (bank, bankStocks) = mkBankRow(0, reservesAtNbp = PLN(100000000), failed = true)
    Banking.reserveInterest(bank, bankStocks, Rate.decimal(575, 4)) shouldBe PLN.Zero
  }
  ...
  "Banking.computeReserveInterest" should "sum per-bank interest" in {
    val rows   = Vector(
      mkBankRow(0, reservesAtNbp = PLN(100000000)),
      mkBankRow(1, reservesAtNbp = PLN(50000000)),
    )
    val result = Banking.computeReserveInterest(banks(rows), stocks(rows), Rate.decimal(6, 2))
  ...
  "Banking.computeStandingFacilities" should "compute deposit facility income for banks with excess reserves" in {
    ...
    val (_, bankStocks) = mkBankRow(0, reservesAtNbp = PLN(100000000))
    val refRate         = BigDecimal("0.0575")
    val depositRate     = DecimalMath.max(BigDecimal("0.0"), refRate - BigDecimal("0.01")) // 4.75%
    val expectedMonthly = decimal(bankStocks.reserve) * depositRate / BigDecimal("12.0")
    // Direct formula check: reservesAtNbp × (refRate − spread) / 12
    expectedMonthly shouldBe (BigDecimal("1e8") * BigDecimal("0.0475") / BigDecimal("12.0") +- BigDecimal("1.0"))
  }
  ...
  it should "charge lombard rate for interbank borrowers (formula check)" in {
    ...
    val refRate             = BigDecimal("0.0575")
    val lombardRate         = refRate + BigDecimal("0.01") // 6.75%
    val interbankBorrowing  = BigDecimal("1e8")
    val expectedMonthlyCost = (interbankBorrowing * lombardRate / BigDecimal("12.0")) * -BigDecimal("1.0")
    // Direct formula check: -|interbankNet| × lombardRate / 12
    expectedMonthlyCost shouldBe (-BigDecimal("1e8") * BigDecimal("0.0675") / BigDecimal("12.0") +- BigDecimal("1.0"))
    expectedMonthlyCost should be < BigDecimal("0.0")
  }

  it should "return zero for failed banks" in {
    val rows   = Vector(mkBankRow(0, reservesAtNbp = PLN(100000000), failed = true))
    val result = Banking.computeStandingFacilities(banks(rows), stocks(rows), Rate.decimal(6, 2))
  ...
  "Banking.interbankInterestFlows" should "compute interest on net positions" in {
    val rows   = Vector(
      mkBankRow(0, interbankNet = PLN(100000000)), // Lender: +100M
      mkBankRow(1, interbankNet = PLN(-100000000)), // Borrower: -100M
    )
    val result = Banking.interbankInterestFlows(banks(rows), stocks(rows), Rate.decimal(6, 2))
    ...
    decimal(result.total) shouldBe (BigDecimal("0.0") +- BigDecimal("0.01"))
  }

  it should "sum to zero for balanced interbank positions" in {
    val rows   = Vector(
      mkBankRow(0, interbankNet = PLN(50000000)),
      mkBankRow(1, interbankNet = PLN(-30000000)),
      mkBankRow(2, interbankNet = PLN(-20000000)),
    )
    val result = Banking.interbankInterestFlows(banks(rows), stocks(rows), Rate.decimal(55, 3))
    decimal(result.total) shouldBe (BigDecimal("0.0") +- BigDecimal("0.01"))
  }
  ...
  it should "return zero for failed banks" in {
    val rows   = Vector(
      mkBankRow(0, interbankNet = PLN(100000000), failed = true),
      mkBankRow(1, interbankNet = PLN(-100000000)),
    )
    val result = Banking.interbankInterestFlows(banks(rows), stocks(rows), Rate.decimal(6, 2))
  ...
  it should "scale linearly with rate" in {
    val rows    = Vector(
      mkBankRow(0, interbankNet = PLN(100000000)),
      mkBankRow(1, interbankNet = PLN(-100000000)),
    )
    val result1 = Banking.interbankInterestFlows(banks(rows), stocks(rows), Rate.decimal(6, 2))
    val result2 = Banking.interbankInterestFlows(banks(rows), stocks(rows), Rate.decimal(12, 2))
    decimal(result2.perBank(0)) shouldBe (decimal(result1.perBank(0)) * BigDecimal("2.0") +- BigDecimal("0.01"))
  }
  ...
  it should "pass with all three monetary plumbing flows combined" in {
    val prev              = zeroSnap.copy(bankCapital = PLN(100000000), bankDeposits = PLN(1000000000), bankLoans = PLN(500000000))
    val resInt            = PLN(200000)
    val sfInc             = PLN(50000)
    val ibInt             = PLN(-1000) // small net from rounding
    val expectedCapChange = (resInt + sfInc + ibInt) * Share.decimal(3, 1)
    val curr              = prev.copy(bankCapital = prev.bankCapital + expectedCapChange)
  ...
  "MonetaryAggregates.compute" should "compute M0/M1/M2/M3 from bank vector" in {
    val (b0, b0Stocks0) = mkBankRow(0, deposits = PLN(600000000), reservesAtNbp = PLN(50000000))
    val b0Stocks        = b0Stocks0.copy(
      demandDeposit = PLN(360000000),
      termDeposit = PLN(240000000),
    )
    val (b1, b1Stocks0) = mkBankRow(1, deposits = PLN(400000000), reservesAtNbp = PLN(50000000))
    val b1Stocks        = b1Stocks0.copy(
      demandDeposit = PLN(240000000),
      termDeposit = PLN(160000000),
    )
    val agg             = Banking.MonetaryAggregates.computeFromBankStocks(Vector(b0, b1), Vector(b0Stocks, b1Stocks), PLN(100000000), PLN(50000000))
    agg.m0 shouldBe PLN(100000000)                                                                            // sum of reserves
    agg.m1 shouldBe PLN(600000000)                                                                            // sum of demand deposits
    agg.m2 shouldBe PLN(1000000000)                                                                           // demand + term
    decimal(agg.m3) shouldBe (BigDecimal("1e9") + BigDecimal("1e8") + BigDecimal("5e7") +- BigDecimal("1.0")) // M2 + tfiAum + corpBonds
  }

  it should "compute credit multiplier as M2/M0" in {
    val (b, stocks0) = mkBankRow(0, deposits = PLN(4500000000L), reservesAtNbp = PLN(1000000000))
    val bankStocks   = stocks0.copy(
      demandDeposit = PLN(2700000000L),
      termDeposit = PLN(1800000000),
    )
    val agg          = Banking.MonetaryAggregates.computeFromBankStocks(Vector(b), Vector(bankStocks), PLN.Zero, PLN.Zero)
    agg.creditMultiplier shouldBe Multiplier.decimal(45, 1)
  }

  it should "handle zero reserves with floor" in {
    val (b, stocks0) = mkBankRow(0, deposits = PLN(1000000000), reservesAtNbp = PLN.Zero)
    val bankStocks   = stocks0.copy(
      demandDeposit = PLN(600000000),
      termDeposit = PLN(400000000),
    )
    val agg          = Banking.MonetaryAggregates.computeFromBankStocks(Vector(b), Vector(bankStocks), PLN.Zero, PLN.Zero)
    agg.creditMultiplier shouldBe Multiplier(1000000000) // m2 / max(PLN(1), PLN.Zero)
  }
  ...
  "Sfc Identity 2" should "include JST deposit change" in {
    val prev   = zeroSnap.copy(bankCapital = PLN(100000000), bankDeposits = PLN(1000000000), bankLoans = PLN(500000000))
    val jstDep = PLN(50000) // positive = JST adds to bank deposits
  ...
  "Sfc Identity 7" should "pass when JST debt change matches" in {
    val prev      = zeroSnap.copy(bankCapital = PLN(100000000), bankDeposits = PLN(1000000000), bankLoans = PLN(500000000))
    val jstSpend  = PLN(10000000)
    val jstRev    = PLN(9800000)
  ...
  it should "fail when JST debt change mismatches" in {
    val prev   = zeroSnap.copy(
      bankCapital = PLN(100000000),
      bankDeposits = PLN(1000000000),
      bankLoans = PLN(500000000),
      jstDebt = PLN(1000),
    )
    // JST debt goes up by 5000 but flows say zero
    val curr   = prev.copy(jstDebt = prev.jstDebt + PLN(5000))
    val flows  = zeroFlows
