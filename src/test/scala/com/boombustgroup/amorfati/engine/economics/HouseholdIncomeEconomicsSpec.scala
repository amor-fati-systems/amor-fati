...
class HouseholdIncomeEconomicsSpec extends AnyFlatSpec with Matchers:
  ...
  it should "count same-month approved consumer loans against projected bank CAR" in {
    ...
    val first  = supply(Banking.CreditRequest(BankId(0), Banking.CreditProduct.ConsumerLoan, PLN(200)), RandomStream.seeded(1))
    val second = supply(Banking.CreditRequest(BankId(0), Banking.CreditProduct.ConsumerLoan, PLN(200)), RandomStream.seeded(2))
