...
object Sfc:
  ...
  case class StockState(
      ...
      quasiFiscalBondsOutstanding: PLN, // BGK/PFR bonds outstanding
      quasiFiscalBankHoldings: PLN,     // commercial-bank BGK/PFR bond holdings
      quasiFiscalNbpHoldings: PLN,      // NBP BGK/PFR bond holdings
      quasiFiscalLoanPortfolio: PLN,    // BGK/PFR subsidized loan portfolio
  )
  ...
  def snapshot(
    ...
    val hhS     = ledgerFinancialState.households.map(_.demandDeposit).sumPln
    val hhD     = ledgerFinancialState.households.map(_.mortgageLoan).sumPln
    ...
    StockState(
      hhSavings = hhS,
      hhDebt = hhD,
      firmCash = ledgerFinancialState.firms.map(_.cash).sumPln,
      firmDebt = ledgerFinancialState.firms.map(_.firmLoan).sumPln,
  ...
  /** Validate exact balance-sheet identities. Returns `Right(())` if all pass,
    ...
    *   8. Consumer credit: Δ consumerLoans = total origination - debtService -
    *      defaultAmount
