...
private[montecarlo] object McHouseholdShortfallCohortSchema:
  ...
  final case class Row(
      ...
      consumerBankRejectedOrigination: PLN,
  ...
  private val columns: Vector[(String, Row => String)] = Vector(
    ...
    "ConsumerBankRejectedOrigination"     -> (row => row.consumerBankRejectedOrigination.format(2)),
    ...
    "ConsumerInsolvencyDefault"           -> (row => row.consumerInsolvencyDefault.format(2)),
  ...
  private def aggregate(
    ...
    Row(
      ...
      consumerBankRejectedOrigination = flow.map(_.consumerBankRejectedOrigination).sumPln,
      ...
      consumerInsolvencyDefault = flow.map(_.consumerInsolvencyDefault).sumPln,
