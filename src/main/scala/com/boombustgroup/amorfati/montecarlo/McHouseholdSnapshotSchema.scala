...
private[montecarlo] object McHouseholdSnapshotSchema:
  ...
  private val columns: Vector[(String, Row => String)] = Vector(
    ...
    "UnmetBasicConsumption"                       -> (row => row.monthlyFlow.unmetBasicConsumption.format(2)),
    "DiscretionaryConsumptionCompression"         -> (row => row.monthlyFlow.discretionaryConsumptionCompression.format(2)),
    ...
    "ConsumerBankPortfolioRiskAdjustedLoanReturn" -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.riskAdjustedLoanReturn)),
    "ConsumerBankPortfolioRiskAdjustedBondReturn" -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.riskAdjustedBondReturn)),
    "ConsumerBankPortfolioWedge"                  -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.wedge)),
    "ConsumerBankPortfolioExpectedLossComponent"  -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.wedgeExpectedLossComponent)),
    "ConsumerBankPortfolioCapitalComponent"       -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.wedgeCapitalComponent)),
    "ConsumerBankPortfolioLevyComponent"          -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.wedgeLevyComponent)),
    "ConsumerBankPortfolioFundingComponent"       -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.wedgeFundingComponent)),
    "ConsumerBankPortfolioPriceShare"             -> (row => portfolioShare(row.monthlyFlow.consumerBankPortfolioChoice, _.priceShareOfWedge)),
    "ConsumerBankPortfolioPriceContribution"      -> (row => portfolioRate(row.monthlyFlow.consumerBankPortfolioChoice, _.wedgePriceContribution)),
    "ConsumerBankPortfolioQuantityThrottle"       -> (row => portfolioShare(row.monthlyFlow.consumerBankPortfolioChoice, _.wedgeQuantityThrottle)),
    ...
    "MortgageArrears"                             -> (row => row.monthlyFlow.mortgageArrears.format(2)),
    "ConsumerDebtArrears"                         -> (row => row.monthlyFlow.consumerDebtArrears.format(2)),
    "TemporaryOverdraft"                          -> (row => row.monthlyFlow.temporaryOverdraft.format(2)),
  ...
  private def portfolioRate(value: Option[BankPortfolioChoiceAudit], select: BankPortfolioChoiceAudit => Rate): String =
    value.fold("")(audit => select(audit).format(6))

  private def portfolioShare(value: Option[BankPortfolioChoiceAudit], select: BankPortfolioChoiceAudit => Share): String =
    value.fold("")(audit => select(audit).format(6))
