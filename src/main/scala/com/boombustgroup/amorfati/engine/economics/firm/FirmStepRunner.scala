...
import com.boombustgroup.amorfati.engine.{MonthWorkflow, OperationalSignals, World}
...
object FirmStepRunner:
  ...
  /** Result of Calvo markup repricing over I-O-adjusted firms. */
  private case class PricingResult(
      firms: Vector[Firm.State], // firms after Calvo markup update
      markupInflation: Rate,     // annualized revenue-weighted markup inflation
  )
  ...
  /** Zero-cost workflow labels for the firm economics step.
    *
    * The step remains synchronous and pure; this DSL only makes the economic
    * ordering explicit at the call site.
    */
  private object FirmEconomicsStepDsl:
    import MonthWorkflow.Program

    def lending(stepIn: StepInput, rng: RandomStream)(using SimParams): Program[LendingConditions] =
      MonthWorkflow.pure(prepareLending(stepIn, rng))
    ...
    def firmProcessing(stepIn: StepInput, lending: LendingConditions, rng: RandomStream)(using SimParams): Program[FirmProcessingResult] =
      MonthWorkflow.pure(processFirms(stepIn.firms, stepIn.ledgerFinancialState, lending, rng, stepIn.traceDecisions))
    ...
    def bondAbsorption(stepIn: StepInput, firmProcessing: FirmProcessingResult, lending: LendingConditions)(using SimParams): Program[BondAbsorptionResult] =
      MonthWorkflow.pure(applyBondAbsorption(firmProcessing, stepIn.w, stepIn.banks, stepIn.ledgerFinancialState, lending.executionMonth))

    def intermediateMarket(stepIn: StepInput, bonded: BondAbsorptionResult)(using SimParams): Program[IntermediateResult] =
      MonthWorkflow.pure(applyIntermediateMarket(bonded.firms, bonded.financialStocks, stepIn))

    def calvoPricing(stepIn: StepInput, intermediate: IntermediateResult, rng: RandomStream)(using p: SimParams): Program[PricingResult] =
    ...
    def laborMarket(stepIn: StepInput, pricing: PricingResult, rng: RandomStream)(using SimParams): Program[LaborMarketResult] =
      MonthWorkflow.pure(processLaborMarket(pricing.firms, stepIn, rng))

    def staffing(pricing: PricingResult, laborMarket: LaborMarketResult): Program[Vector[Firm.State]] =
      MonthWorkflow.pure(LaborMarket.syncFirmStaffing(pricing.firms, laborMarket.households))
  ...
  private case class StepInput(
      ...
      fiscal: FiscalConstraintEconomics.StepOutput,         // fiscal/lending-base outputs from stage 1
      labor: LaborEconomics.StepOutput,                     // labor-market outputs from stage 2
      householdIncome: HouseholdIncomeEconomics.StepOutput, // household income and balance outputs from stage 3
      demand: DemandEconomics.StepOutput,                   // demand and sector pressure outputs from stage 4
