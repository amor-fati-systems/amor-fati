...
object Expectations:
  ...
  private val MinCredibility             = Share.decimal(1, 2)        // floor on credibility index
  private val OutputGapClamp             = Coefficient.decimal(5, 2)  // ±5 pp unemployment gap clamp
  private val FgBlendWeight              = Share.decimal(6, 1)        // weight on forward guidance in expected rate
  private val UndershootLearningWeight   = Coefficient.decimal(35, 2) // below-target inflation updates expectations more slowly
  private val UndershootCredibilityScale = Share.decimal(35, 2)       // below-target inflation erodes credibility less than overshooting
  private val MaxTargetUndershoot        = Rate.decimal(3, 2)         // expectations can drift at most 3pp below target at zero credibility
