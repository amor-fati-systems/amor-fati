...
/** Diaspora remittance inflows from Polish workers abroad.
  *
  * Models the inbound remittance channel calibrated to NBP BoP bridge prior
  * (~18 mld PLN/year). Remittances enter as household income, with exchange
  * rate pass-through and counter-cyclical sensitivity to excess unemployment.
  * Affects SFC Identities 2 (bank balance sheet) and 4 (external balance).
  *
  * @param perCapita
  *   monthly remittance per diaspora member (PLN, NBP BoP bridge prior: ~40
  *   PLN/os/month)
  * @param erElasticity
