...
object EmpiricalValidationExport:
  ...
  private def writeArtifactsZIO(
    ...
    for
      sourcePath      <- TsvFile.writeAll(sourceManifestPath, sourceRows, SourceManifestTsvSchema)(DiagnosticIo.outputFailure)
      runPath         <- TsvFile.writeAll(runManifestPath, Vector(modelRun), ModelRunManifestTsvSchema)(DiagnosticIo.outputFailure)
      snapshotTsvPath <- TsvFile.writeAll(snapshotPath, snapshot, SnapshotTsvSchema)(DiagnosticIo.outputFailure)
  ...
  private def readTsv(path: Path): Either[String, Vector[TsvRow]] =
    TsvRows.readRows(path)

  private[diagnostics] lazy val SourceManifestTsvSchema: TsvSchema[SourceManifestRow] =
    TsvSchema.fromCells(SourceManifestHeader): row =>
      Vector(
        row.target,
        row.sourceProvider,
        row.sourceUrl,
  ...
  private[diagnostics] lazy val ModelRunManifestTsvSchema: TsvSchema[ModelRunManifest] =
    TsvSchema.fromCells(ModelRunManifestHeader): manifest =>
      Vector(
        manifest.runId,
  ...
  private[diagnostics] lazy val SnapshotTsvSchema: TsvSchema[SnapshotRow] =
    TsvSchema.fromCells(SnapshotHeader): row =>
      Vector(
        row.target,
        row.sourceProvider,
        row.sourceUrl,
