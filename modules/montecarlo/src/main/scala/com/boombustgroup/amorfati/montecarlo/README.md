...
Monte Carlo TSV adapters, console progress reporting, and typed result/error
...
| `io` | `McTsvFile`, `McTsvSchema` | Monte Carlo TSV contracts backed by the shared `com.boombustgroup.amorfati.tsv.Tsv*` layer |
...
prefer them for Monte Carlo artifacts and use `TsvFile`, `TsvSchema`, and
`TsvRows` from `com.boombustgroup.amorfati.tsv` directly only when it needs the
plain TSV primitive.
