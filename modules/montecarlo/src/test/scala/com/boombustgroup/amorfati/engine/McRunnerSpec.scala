...
class McRunnerSpec extends AnyFlatSpec with Matchers:
  ...
  it should "keep adoption ratio in [0, 1]" in {
    for t <- 0 until duration do
      metricDecimal(row(t)(Col.TotalAdoption.ordinal)) should be >= BigDecimal("0.0")
      metricDecimal(row(t)(Col.TotalAdoption.ordinal)) should be <= BigDecimal("1.0")
  }

  it should "keep unemployment in [0, 1]" in {
    for t <- 0 until duration do
      metricDecimal(row(t)(Col.Unemployment.ordinal)) should be >= BigDecimal("0.0")
      metricDecimal(row(t)(Col.Unemployment.ordinal)) should be <= BigDecimal("1.0")
  }
  ...
  it should "have positive sigma values" in {
    for t <- 0 until duration; s <- 0 until 6 do metricDecimal(row(t)(Col.sectorSigma(s).ordinal)) should be > BigDecimal("0.0")
  }

  it should "have positive mean degree" in {
    for t <- 0 until duration do metricDecimal(row(t)(Col.MeanDegree.ordinal)) should be > BigDecimal("0.0")
  }

  it should "keep price level above floor (0.30)" in {
    for t <- ts.indices do
      withClue(s"Month ${t + 1}: ") {
        metricDecimal(row(t)(Col.PriceLevel.ordinal)) should be >= BigDecimal("0.30")
      }
  }
  ...
  it should "keep GovBondMarketYield non-negative after month 1" in {
    for t <- 1 until duration do
      withClue(s"Month ${t + 1}: ") {
        metricDecimal(row(t)(Col.GovBondMarketYield.ordinal)) should be >= BigDecimal("0.0")
      }
  }
  ...
  it should "satisfy bond clearing identity at every month" in {
    for t <- ts.indices do
      ...
      withClue(s"Month ${t + 1}: holders($holders) vs outstanding($outstanding): ") {
        metricDecimal(holders) shouldBe metricDecimal(outstanding) +- BigDecimal("1.0")
      }
  }
  ...
  it should "have non-negative unemployment benefits" in {
    for t <- ts.indices do
      withClue(s"Month ${t + 1}: ") {
        metricDecimal(row(t)(Col.UnempBenefitSpend.ordinal)) should be >= BigDecimal("0.0")
      }
  }
  ...
  it should "cut NBP rate when inflation is negative (symmetric Taylor)" in {
    ...
    val deflationMonth = ts.indices.find(t => metricDecimal(row(t)(Col.Inflation.ordinal)) < BigDecimal("0.0"))
    deflationMonth match
      case Some(t) =>
        val rateAfterDeflation = ((t + 1) until duration).map(m => row(m)(Col.RefRate.ordinal))
        rateAfterDeflation.exists(value => metricDecimal(value) < initialRate) shouldBe true
      case None    =>
        succeed
  }
  ...
  it should "have interbank rate deviating from refRate" in {
    val deviates = ts.indices.exists { t =>
      ...
      (metricDecimal(ibRate) - metricDecimal(refRate)).abs > BigDecimal("0.000001")
    }
